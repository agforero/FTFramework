C$Procedure DLABFS ( DLA, begin forward search )
 
      SUBROUTINE DLABFS ( HANDLE, DESCR, FOUND )
      IMPLICIT NONE
 
C$ Abstract
C
C     Begin a forward segment search in a DLA file.
C
C$ Disclaimer
C
...
C     DESCR      O   Descriptor of first segment in DLA file.
...
C$ Detailed_Output
C
C     DESCR       is the descriptor of the first DLA segment in the
...
C     conduct a forward traversal; all that is necessary is to have
C     access to the first descriptor in the file, which this routine
C     provides.
C
C$ Examples
C
...
C        23-JAN-2013 (NJB)
C
C           Deleted third exception description from the Exceptions
C           header section.
C
C        08-OCT-2009 (NJB)
C
...
C$ Index_Entries
C
C     begin forward search in dla file
C
C-&
...
C
C     Look up the pointer to the first DLA segment descriptor in the
C     file.  Then look up the segment descriptor itself.
C
      CALL DASRDI ( HANDLE, LLBIDX, LLBIDX, THIS )
