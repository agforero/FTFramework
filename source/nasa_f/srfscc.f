C$Procedure SRFSCC (Surface string and body ID code to surface ID code )

      SUBROUTINE SRFSCC ( SRFSTR, BODYID, CODE, FOUND )

C$ Abstract
C
C     Translate a surface string, together with a body ID code, to the
C     corresponding surface ID code. The input surface string may
C     contain a name or an integer ID code.
C     
C$ Disclaimer
C
...
C
C
C     BODYID     is the integer ID code of the body associated with the
C                surface designated by SRFSTR.
C
C$ Detailed_Output
C
C     CODE       is integer ID code of the surface designated by
C                SRFSTR, for the body designated by BODYID, if for this
...
C                corresponding to the input surface string and body ID
C                code was found. FOUND is .FALSE. otherwise.
C
C$ Parameters
C
C     None.
...
C
C     SRFSCC is one of four related subroutines:
C
...
C
C        Translate each surface string and body ID pair to the 
C        associated surface ID code. Also perform a translation
C        for a surface name having no matching ID.
C                    
...
C
C
C          DATA  ( SRFSTR(I), BODYID(I), I = 1, NCASE )  /
C         .
C         .        'MGS MOLA  64 pixel/deg',    499,
C         .        'PHOBOS GASKELL Q512',       401,
C         .        'MGS MOLA 128 pixel/deg',    499,
C         .        '1',                         499,
C         .        '1',                         401,
C         .        '2',                         499,
C         .        'ZZZ',                       499      /
...
C          DO I = 1, NCASE
C
C             CALL SRFSCC ( SRFSTR(I), BODYID(I),
C         .                 SURFID,    FOUND     )
C
...
C$ Index_Entries
C
C     surface string and body ID code to surface ID code 
C
C-&
...
C
C     Try to translate the input surface name to a known surface ID
C     code.
C      
...
C        integer, for example, '999'.  If so, find the equivalent datum
C        of INTEGER type.
C
         CALL NPARSI ( SRFSTR, CODE, ERRMSG, PTR )
C
C        If the string parsed as an integer, PTR is zero; otherwise
C        it's non-zero.
C
         FOUND = PTR .EQ. 0
