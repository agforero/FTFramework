...
SUBROUTINE polzeros (n, poly, eps, big, small, nitmax, root, radius, err, iter)
INTEGER, INTENT(IN)      :: n, nitmax
INTEGER, INTENT(OUT)     :: iter
TYPE (qc), INTENT(IN)    :: poly(:)
TYPE (qc), INTENT(OUT)   :: root(:)
TYPE (quad), INTENT(OUT) :: radius(:)
REAL (dp), INTENT(IN)    :: eps, small, big
LOGICAL, INTENT(OUT)     :: err(:)

! Local variables
INTEGER                :: i, nzeros
TYPE (qc)              :: corr, abcorr
TYPE (quad)            :: amax, apoly(n+1), apolyr(n+1)
...
DO i = 1, n+1
  apolyr(n-i+2) = eps*apoly(i)*(3.8_dp*(n-i+1) + 1._dp)
  apoly(i) = eps*apoly(i)*(3.8_dp*(i-1) + 1._dp)
END DO
...
DO i = 1,n
  err(i) = .true.
  IF (ABS( radius(i)%hi + 1.0_dp) < eps) err(i) = .false.
END DO
...
INTEGER, INTENT(IN)      :: n
TYPE (qc), INTENT(IN)    :: poly(:), z
TYPE (qc), INTENT(OUT)   :: corr
TYPE (quad), INTENT(IN)  :: apoly(:), apolyr(:)
...
IF (az%hi <= 1.0_dp) THEN
  ...
  IF (.NOT.again) radius = DBLE(n)*(absp + ap) / ABS(p1)
  RETURN
ELSE
...
! and use formula (2) for p(z)/p'(z). Analogously do for the inclusion radius.
  ...
  again = (absp > quad(small, 0.0_dp) + ap)
  ppsp = (p*z)/p1
  den = qc(quad(DBLE(n),0.0_dp), zero) * ppsp - qc(one,zero)
  corr = z*(ppsp/den)
  IF (again)RETURN
...
TYPE (qc), INTENT(OUT)      :: y(:)
REAL (dp), INTENT(IN)       :: small, big
TYPE (quad), INTENT(IN OUT) :: a(:)
TYPE (quad), INTENT(OUT)    :: radius(:)
...
! Check if the modulus is too small
    IF ((temp%hi < -xbig) .AND. (temp%hi >= xsmall)) THEN
      ...
      WRITE(2,*)'REPRESENT THEIR INVERSES AS TYPE (qc), THEY'
    ...
    IF (temp%hi < xsmall) THEN
      ...
      WRITE(*,*)'REPRESENTED AS TYPE (qc), THEY ARE SET TO 0'
      WRITE(*,*)'THE CORRESPONDING RADII ARE SET TO -1'
      WRITE(2,*)'WARNING: ',nzeros,' ZERO(S) ARE TOO SMALL TO BE'
      WRITE(2,*)'REPRESENTED AS TYPE (qc), THEY ARE SET 0'
...
! Check if the modulus is too big
    IF (temp%hi > xbig) THEN
      ...
      WRITE(*,*)'REPRESENTED AS TYPE (qc),'
      WRITE(*,*)'THE CORRESPONDING RADII ARE SET TO -1'
      WRITE(2,*)'WARNING: ',nzeros, ' ZERO(S) ARE TOO BIG TO BE'
      WRITE(2,*)'REPRESENTED AS TYPE (qc),'
      WRITE(2,*)'THE CORRESPONDING RADII ARE SET TO -1'
    END IF
...
! Compute NZEROS approximations equally distributed in the disk of radius R
    ...
    DO j = iold, i-1
      jj = j - iold + 1
      IF (r%hi <= 1.0_dp/big .OR. r == quad(big, 0.0_dp))   &
...
! Local variables
TYPE (quad)          :: s1, s2
REAL (dp), PARAMETER :: toler = 0.4_dp
