...
! Using Lahey's LF90, this MUST be compiled with optimization OFF.
! i.e. using LF90 quad -o0
! OK with ELF90 (no optimization).
! Use quad_df for the Lahey/Fujitsu LF95 compiler.
...
REAL (dp), PARAMETER :: constant = 2._dp**(nbits + 11 - nbits/2) + 1._dp
                        !                          ^^
                        ! N.B. IEEE format for double precision uses 53 bits
                        ! for the mantissa.   PC floating-point processors
                        ! use an extra 11 bits - hence that 11.
