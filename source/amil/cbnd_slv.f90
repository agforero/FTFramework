MODULE Cmplx_Banded_Solve
! Solve systems of complex linear banded equations.
...
! N.B. cbslv requires ml extra columns to be added at the right-hand side.
...
!
INTEGER, INTENT(IN)           :: n
COMPLEX (dp), INTENT(IN OUT)  :: dx(:)
INTEGER, INTENT(IN)           :: incx
COMPLEX (dp), INTENT(IN OUT)  :: dy(:)
...
SUBROUTINE cbslv(m0, a, ka, n, ml, mu, b, ierr)
...
!
!
!  AFTER AN INITIAL CALL TO CBSLV, THE ROUTINE MAY BE RECALLED WITH M0.NE.0
!  FOR A NEW B.  WHEN M0.NE.0 IT IS ASSUMED THAT A,KA,N,ML,MU,IWK HAVE NOT
!  BEEN MODIFIED.  CBSLV RETRIEVES THE LU DECOMPOSITION WHICH WAS OBTAINED
!  ON THE INITIAL CALL TO CBSLV AND SOLVES THE NEW EQUATIONS AX = B.
!  IN THIS CASE IERR IS NOT REFERENCED.
! ----------------------------------------------------------------------

INTEGER, INTENT(IN)           :: m0
COMPLEX (dp), INTENT(IN OUT)  :: a(:,:)   ! a(ka,*)
...
!
  CALL cbfa(a, n, ml, mu, iwk, ierr)
  IF (ierr /= 0) RETURN
END IF
!
!     SOLVE THE SYSTEM OF EQUATIONS
!
CALL cbsl(a, n, ml, mu, iwk, b, 0)
RETURN
!
!     ERROR RETURN
!
...
30 ierr = -3
RETURN
END SUBROUTINE cbslv



SUBROUTINE cbfa(a, n, ml, mu, ipvt, info)
...
!  ON RETURN
!
!     A       AN UPPER TRIANGULAR MATRIX IN BAND STORAGE AND THE MULTIPLIERS
...
!                 SUBROUTINE, BUT IT DOES INDICATE THAT CBSL WILL DIVIDE BY
...
COMPLEX (dp), INTENT(IN OUT)  :: a(:,:)   ! a(lda,*)
INTEGER, INTENT(IN)           :: n
INTEGER, INTENT(IN)           :: ml
INTEGER, INTENT(IN)           :: mu
...
!
    ll = ml1 + k - l
    IF (l /= k) CALL zswap(mp+1, a(k,ml1:), 1, a(l,ll:), 1)
!
!     SKIP COLUMN REDUCTION IF PIVOT IS ZERO
!
    IF (a(k,ml1) == (0.0_dp, 0.0_dp)) THEN
      info = k
    ELSE
!
!     COMPUTE MULTIPLIERS
...
!
  ipvt(n) = n
  IF (a(n,ml1) == (0.0_dp, 0.0_dp)) info = n
  RETURN
END IF
!
!     CASE WHEN ML = 0
...
SUBROUTINE cbsl(a, n, ml, mu, ipvt, b, job)
...
!     OCCUR IF CBFA AND CBSL ARE CALLED CORRECTLY AND CBFA HAS SET INFO = 0.
!
!  WRITTEN BY E.A. VOORHEES, LOS ALAMOS SCIENTIFIC LABORATORY.
!  MODIFIED BY A.H. MORRIS, NAVAL SURFACE WEAPONS CENTER.
!
...
END MODULE Cmplx_Banded_Solve



PROGRAM t_cbslv
USE Cmplx_Banded_Solve
IMPLICIT NONE
