...
!  Local Parameters:
!
!    Local, integer DEG_MAX, the highest degree of polynomial to be handled.  
!
!    Local, integer Q_MAX, the order of the largest field to be handled.
!
!  Global Parameters:
!
!    The common block /FIELD/ contains:
!
!    Common, integer Q, the order of the field.
!
!    Common, integer P, the characteristic of the field.
!
!    Common, integer ADD(0:Q_MAX-1,0:Q_MAX-1), the field addition table. 
!
!    Common, integer MUL(0:Q_MAX-1,0:Q_MAX-1), the field multiplication table. 
!
!    Common, integer SUB(0:Q_MAX-1,0:Q_MAX-1), the field subtraction table.
!
  implicit none
...
!  Parameters:
!
!    Input, integer N, the value being sought.
!
!    Input, integer TAB(*), the table to be searched.
!
!    Input, integer I, TAB_MAX, the first and last entries of
!    TAB to be examined.
!
!    Output, integer FIND, is the index ( between I and TAB_MAX) of the 
...
!  Parameters:
!
!    Input, integer OUNIT, the unit number of the output file.
!
!    Input, integer Q_INIT, the order of the field.
!
!  Local Parameters:
!
!    Local, integer SIEVE_MAX, the size of the sieve.  
!
!    Array MONPOL holds monic polynomials.
!
!    Array SIEVE says whether the polynomial is still OK.
!
!    Local, integer DEG_MAX, the highest degree of polynomial
!    to be handled.  
!
!    Local, integer Q_MAX, the order of the largest field to
!    be handled.
!
!    Local, integer NPOLS, the number of irreducible polynomials to
...
!
  ...
  integer, parameter :: sieve_max = 400
...
!  Parameters:
!
!    Input, integer IN, the integer to be decoded.
!
!    Output, integer POLY(-1:DEG_MAX), the polynomial in the field
...
!  Parameters:
!
!    Input, integer PA(-1:DEG_MAX), PB(-1:DEG_MAX), the polynomials
!    to be multiplied.  Their degree is stored in entry (-1).
!
!    Output, integer PC(-1:DEG_MAX), the product of PA and PB,
...
!  Parameters:
!
!    Input, integer POLY(-1:DEG_MAX), the polynomial.
!
!    Output, integer PTOI, the corresponding integer.
!
!  Local Parameters:
!
!    Local, integer DEG_MAX, the highest degree of polynomial to be handled.
