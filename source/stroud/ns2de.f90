...
!
!    Input, real ( kind = 8 ) U(N), V(N), P(N), the velocity and pressure.
!
...
!
!    Input, real ( kind = 8 ) RHO, the density.
!
!    Input, integer ( kind = 4 ) N, the number of evaluation points.
!
...
!
!    Input, real ( kind = 8 ) X(N), Y(N), the coordinates of nodes.
!
!    Input, real ( kind = 8 ) T, the current time.
!
!    Input, real ( kind = 8 ) U(N), V(N), the X and Y velocity.
!
!    Input, real ( kind = 8 ) P(N), the pressure.
!
!    Output, real ( kind = 8 ) UR(N), the U-equation residual.
!
!    Output, real ( kind = 8 ) VR(N), the V-equation residual.
!
!    Output, real ( kind = 8 ) PR(N), the P-equation residual.
!
  implicit none
...
!    Input, real ( kind = 8 ) T, the current time.
!
!    Output, real ( kind = 8 ) F(N), G(N), H(N), the right hand sides.
!
  implicit none
...
!    Input, real ( kind = 8 ) T, the time coordinate or coordinates.
!
!    Output, real ( kind = 8 ) F(N), G(N), H(N), the right hand sides of 
...
!    Input, real ( kind = 8 ) T, the current time.
!
!    Output, real ( kind = 8 ) U(N), V(N), the X and Y velocity.
!
!    Output, real ( kind = 8 ) P(N), the pressure.
!
  implicit none
