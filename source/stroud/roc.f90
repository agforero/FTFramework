...
!
  ...
  logical debug
  character ( len = 255 ) debug_file_name
  integer ( kind = 4 ) debug_unit
  character ( len = 35 ) good(max_score)
  character ( len = 255 ) good_file_name
  ...
  character ( len = 255 ) search_file_name
...
!
!    Input, integer ( kind = 4 ) MAX_SCORE_NEG_SELECT, the maximum number of 
!    scores with negative results that will be considered.
!
!    Input, integer ( kind = 4 ) MAX_SCORE_SELECT, the maximum number of scores
...
!
!    Output, integer ( kind = 4 ) NUM_SCORE_NEG_SELECT, the number of selected 
!    scores associated with negative results.
!
!    Input, integer ( kind = 4 ) NUM_SCORE_POS, the number of scores associated
!    with positive results.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_POS_SELECT, the number of selected 
!    scores associated with positive results.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_SKIP, the number of 
!    unselected scores.
!
!    Input, logical PLOT, is TRUE if plots are desired.
!
!    Output, integer ( kind = 4 ) RESULT(MAX_SCORE), the result code.  
!    RESULT(I) is:
...
!
!    Input, real ( kind = 8 ) SCORE_MAX_SELECTED, the maximum score of the 
!    selected data.
!
!    Input, real ( kind = 8 ) SCORE_MIN_SELECTED, the minimum score of the 
!    selected data.
!
...
!
!    Output, integer ( kind = 4 ) MAX_SCORE_NEG_SELECT_HI, 
!    MAX_SCORE_NEG_SELECT_INC, MAX_SCORE_NEG_SELECT_LO, the maximum, increment, 
!    and minimum for the DO loop that determines the number of scores with
...
!    Input, integer ( kind = 4 ) DEBUG_UNIT, the unit number of the debug file.
!
!    Input, integer ( kind = 4 ) MAX_SCORE_NEG_SELECT_HI, 
!    MAX_SCORE_NEG_SELECT_INC, MAX_SCORE_NEG_SELECT_LO, the maximum, increment, 
!    and minimum for the DO loop that determines the number of scores with 
!    negative results that will be considered.
!
...
!    to be created.
!
!    Output, integer ( kind = 4 ) GRAPH_FILE_UNIT, the unit number associated
!    with the graph file.
!
!    Output, integer ( kind = 4 ) IERROR, error flag.
!    0, no error.
...
!    scores associated with positive results.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_SELECT, the number of 
!    selected scores.
!
!    Input, integer ( kind = 4 ) RESULT(MAX_SCORE), the result code.  RESULT(I)
!    is:
!      J, item I is selected as the J-th POSITIVE representative;
!      0, item I is omitted;
!    - J, item I is selected as the J-th NEGATIVE representative.
...
!    Input, real ( kind = 8 ) SCORE(MAX_SCORE), the scores.
!
!    Input, real ( kind = 8 ) SCORE_MAX_SELECTED, SCORE_MIN_SELECTED, the 
!    maximum and minimum scores to be plotted.
!
  implicit none
...
!    Input, integer ( kind = 4 ) NUM_SCORE, the number of scores.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_NEG, the number of scores associated
!    with negative results.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_POS, the number of scores associated
!    with positive results.
!
  implicit none
...
!    Input, integer ( kind = 4 ) NUM_SCORE, the number of scores.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_NEG, the number of scores 
...
!    scores associated with positive results.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_SELECT, the number of selected
!    scores.
!
!    Output, integer ( kind = 4 ) NUM_SCORE_SKIP, the number of unselected
!    scores.
!
!    Output, integer ( kind = 4 ) RESULT(MAX_SCORE), the result code.  
!    RESULT(I) is:
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) IUNIT, the unit number associated with 
!    the open file.
!
...
!
!    Output, integer ( kind = 4 ) VALUE_INDEX(MAX_INDEX), the indices in X 
!    of negative entries.
!
  implicit none
...
!    Input, integer ( kind = 4 ) NUM_SCORE, the number of scores.
!
!    Input, integer ( kind = 4 ) NUM_SCORE_NEG_SELECT, the number of selected 
!    scores associated with negative results.
!
!    Input, integer ( kind = 4 ) NUM_SCORE_POS_SELECT, the number of selected
!    scores associated with positive results.
!
!    Input, integer ( kind = 4 ) RESULT(MAX_SCORE), the result code.  RESULT(I)
!    is:
...
!    - J, item I is selected as the J-th NEGATIVE representative.
!
!    Input, real ( kind = 8 ) ROC_VAL, the value of the receiver operator 
!    characteristic.
!
!    Input, character ( len = * ) SEARCH_FILE_NAME, the name of the search file.
!
...
!    scores associated with positive results.
!
!    Input, integer ( kind = 4 ) RESULT(MAX_SCORE), the result code.  
!    RESULT(I) is:
!      J, item I is selected as the J-th POSITIVE representative;
!      0, item I is omitted;
!    - J, item I is selected as the J-th NEGATIVE representative.
...
!
!    Input, integer ( kind = 4 ) NUM_PLOT, the number of points to draw on each
!    curve.
!
!    Input, integer ( kind = 4 ) NUM_SCORE, the number of scores.
!
...
!    - J, item I is selected as the J-th NEGATIVE representative.
!
!    Output, real ( kind = 8 ) S_MAX_DIFF, the score at which the maximum 
!    difference between the positive and negative curves occurs.
!
!    Input, real ( kind = 8 ) SCORE(MAX_SCORE), the scores.
!
!    Input, real ( kind = 8 ) SCORE_MAX_SELECTED, SCORE_MIN_SELECTED, the 
...
!*****************************************************************************80
!
!! PNS_GRAPH_FILE_WRITE creates a positive/negative/score graphics file.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!    Input, integer ( kind = 4 ) NUM_GOOD, the number of "good" identifiers.
!
!    Input, integer ( kind = 4 ) NUM_PLOT, the number of points to draw on 
!    each curve.
!
!    Input, integer ( kind = 4 ) NUM_SCORE_NEG_SELECT, the number of selected
!    scores associated with negative results.
...
!
!    Input, integer ( kind = 4 ) NBIN, the number of bins.  Two extra bins, 
!    #0 and #NBIN+1, count extreme values.
!
...
!*****************************************************************************80
!
!! SVECI_SEARCH_BINARY_A searches an ascending sorted string vector.
!
!  Discussion:
!
!    The strings are implicitly capitalized.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
