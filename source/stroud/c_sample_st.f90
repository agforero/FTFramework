...
!*****************************************************************************80
!
!! C_SAMPLE_ST calls SUPERLU to solve a system.
!
!  Licensing:
!
...
!
  ...
  complex ( kind = 4 ), allocatable :: acc(:)
  complex ( kind = 4 ), dimension ( nst ) :: ast = (/ &
...
!
  ...
  if ( info /= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'C_SAMPLE_ST - Fatal error!'
...
!
!    Input, complex ( kind = 4 ) AST(NST), the ST values.
!
...
!
!    Output, complex ( kind = 4 ) ACC(NCC), the CC values.
!
  implicit none
