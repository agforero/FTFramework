...
!
  if ( num_arg < 1 ) then
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) &
      '  Convert simple plot descriptions into a PostScript file.'
    write ( *, '(a)' ) ' '
...
!  Parameters
!
!    Input, integer ( kind = 4 ) FILEIN_UNIT, the unit number associated with 
!    the input file.  This will be 0 in the case of an interactive session, 
!    when input comes from the user, and not a file.
!
...
!
  ...
  real ( kind = 8 ) degrees_to_radians
  integer ( kind = 4 ) filein_badlines
  integer ( kind = 4 ) filein_linecount
  integer ( kind = 4 ) filein_unit
  character ( len = * ) fileout_name
  integer ( kind = 4 ) fileout_unit
  real ( kind = 8 ) g
...
!  Parameters
!
!    Input/output, integer ( kind = 4 ) FILEIN_LINECOUNT, the number of lines
!    of input that were read.
!
!    Input, integer ( kind = 4 ) FILEIN_UNIT, the unit number associated with 
!    the input file.  This will be 0 in the case of an interactive session, 
!    when input comes from the user, and not a file.
...
!
  ...
  integer ( kind = 4 ), parameter :: maxpoly = 4
...
!  Parameters
!
!    Input/output, integer ( kind = 4 ) FILEIN_LINECOUNT, the number of lines of
