...
!*****************************************************************************80
!
!! S_SAMPLE_ST calls SUPERLU to solve a system in sparse triplet format.
!
!  Licensing:
!
...
!
  ...
  real ( kind = 4 ), allocatable :: acc(:)
  real ( kind = 4 ), dimension ( nst ) :: ast = (/ &
    19.0, 12.0, 12.0, &
    21.0, 12.0, 12.0, &
    21.0, 16.0, &
    21.0,  5.0, &
...
!
  ...
  if ( info /= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'S_SAMPLE_ST - Fatal error!'
...
!
!    Input, real ( kind = 4- ) AST(NST), the ST values.
!
...
!
!    Output, real ( kind = 4 ) ACC(NCC), the CC values.
!
  implicit none
