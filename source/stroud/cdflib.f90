...
!
!    Output, real ( kind = 8 ) gamma_user, the value of the Gamma function.
!
  implicit none
...
!
    ...
    gamma_user = exp ( w )* ( 1.0D+00 + t )

    if ( a < 0.0D+00 ) then
      gamma_user = ( 1.0D+00 / ( gamma_user * s ) ) / x
    end if
