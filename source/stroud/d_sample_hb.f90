...
!*****************************************************************************80
!
!! D_SAMPLE_HB is a sample program for SUPERLU.
!
!  Discussion:
!
!    The matrix is read from a Harwell-Boeing (HB) file.
!
!    The HB format already stores the matrix information in 
!    compressed column (CC) form.
!
!  Licensing:
!
...
!
  ...
  real ( kind = 8 ) acc(ncc_max)
  real ( kind = 8 ) b(n_max)
  real ( kind = 8 ) b2(n_max)
...
!
  call dhbcode1 ( n, n, ncc, acc, icc, ccc )
  m = n
!
!  Print the matrix.
!
...
!
  ...
  if ( info /= 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'D_SAMPLE_HB - Fatal error!'
...
subroutine dhbcode1 ( m, n, ncc, acc, icc, ccc )

!*****************************************************************************80
!
!! DHBCODE1 is a sample FORTRAN90 code to read a sparse matrix in HB format.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
!    Output, real ( kind = 8 ) ACC(NCC), the nonzero values.
!
