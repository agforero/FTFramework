...
!    of target points.
!
!    Local, integer ( kind = 4 ) FAMILY, specifies the desired family of Padua points.
!
...
!
  ...
  integer ( kind = 4 ), parameter :: ntgmax = ntg1mx ** 2
...
!
!    Input, real ( kind = 8 ) A1, B1, A2, B2, the coordinates of the extreme
!    corners of the rectangle.
!
!    Input, integer ( kind = 4 ) FAMILY, DEG, select the family of Padua points at 
!    degree DEG.
!
...
!
  ...
  theta = real ( 2 * mod ( deg, 2 ) - 1, kind = 8 ) &
    * real ( family - 1, kind = 8 ) * pi / 2.0D+00
...
!
  ...
  do i = 1, ntg1
    do j = 1, ntg1
      ntg = ntg + 1
      tg1(ntg) = a1 + real ( j - 1, kind = 8 ) * ( b1 - a1 ) &
        / real ( ntg1 - 1, kind = 8 ) 
      tg2(ntg) = a2 + real ( i - 1, kind = 8 ) * ( b2 - a2 ) &
        / real ( ntg1 - 1, kind = 8 )
    end do
  end do
