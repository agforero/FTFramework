...
!*****************************************************************************80
!
!! ORBITAL_CONTOUR uses DISLIN to display a contour plot of Z(X,Y) data.
!
!  Licensing:
!
...
! use dislin
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'ORBITAL_CONTOUR:'
  write ( *, '(a)' ) '  FORTRAN90 version'
  write ( *, '(a)' ) '  Use DISLIN to make a contour plot of Z(X,Y) data.'
!
!  Read the data.
!
  call r4mat_header_read ( 'orbital_data.txt', m, nn )

  allocate ( xyz(1:m,1:n) )

  call r4mat_data_read ( 'orbital_data.txt', m, nn, xyz )
!
!  Split the data.
...
!
!  Z is a table.
!  The first dimension should contain values for constant X.
!
  k = 0
  do j = 1, n
    do i = 1, n
  ...
  zmax = maxval ( z(1:n,1:n) )
  zmin = minval ( z(1:n,1:n) )
!
...
!
  call setfil ( 'orbital_contour.png' )
!
...
!
  call graf ( xmin, xmax, xmin, 1.0, ymin, ymax, ymin, 1.0 )
!
...
!  Define the title.
!
  call titlin ( 'Orbital contour plot', 1 )
  call title ( )
!
...
!
  ...
  do level = 1, level_num
    ...
    level_value = real ( ( real ( level_num + 1 - level, kind = 8 ) * zmin   &
                  + real (                 level, kind = 8 ) * zmax ) &
                  / real ( level_num + 1,         kind = 8 ), kind = 4 )

    call contur ( x, n, y, n, z, level_value )
