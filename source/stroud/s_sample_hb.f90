...
!*****************************************************************************80
!
!! S_SAMPLE_HB calls SUPERLU to solve a system stored in a Harwell-Boeing file.
!
!  Licensing:
!
...
!
  ...
  real ( kind = 4 ) acc(ncc_max)
  real ( kind = 4 ) b(n_max)
  real ( kind = 4 ) b2(n_max)
...
!
  call shbcode1 ( n, n, ncc, acc, icc, ccc )
  m = n
...
!
!    Output, real ( kind = 4 ) B(M), the product A*X.
!
  implicit none
...
!    Input, integer ( kind = 4 ) CCC(N+1), the compressed CC columns.
!
!    Input, real ( kind = 4 ) ACC(NCC), the CC values.
!
...
!*****************************************************************************80
!
!! SHBCODE1 is a sample FORTRAN90 code to read a sparse matrix in HB format.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
!    Output, real ( kind = 4 ) ACC(NCC), the nonzero values.
!
