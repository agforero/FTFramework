...
!
!    26 September 2013
!    Converted to REAL ( KIND = 8 ) arithmetic.
!
!    09 October 2000:   
...
!
  ...
  integer ( kind = 4 ), parameter :: maxcol = 30
  integer ( kind = 4 ), parameter :: maxrow = 16

  real ( kind = 8 ) a(maxrow,maxcol)
  ...
  write ( *, '(a)' ) '  Last modified on 26 September 2013.'
...
!
      ...
      call la_opt ( a, a_int, iabot, iatop, iform, nrow, ncol )
!
...
!
      ...
      call col_add ( a, a_int, iatop, iabot, ierror, base, iform, icol1, &
...
!
      ...
      call col_div ( a, a_int, iatop, iabot, ierror, iform, icol, &
...
!
      ...
      call row_add ( a, a_int, iatop, iabot, ierror, base, iform, irow1, &
...
!
!    Input, integer ( kind = 4 ) IFORM, 0/1/2/3/4, 
!    rat/real/dec/int/mod arithmetic.
!
!    Input, integer ( kind = 4 ) NROW, NCOL, the number of rows and columns.
!
!    Input/output, real ( kind = 8 ) A(NROW,NCOL), the current real matrix.
!
...
!
  ...
  if ( iform == 0 ) then
    ...
    call r8_to_s_left ( a(irow,icol), chrtmp3 )
...
!  Parameters:
!
!    Input/output, real ( kind = 8 ) A(NROW,NCOL), the current matrix.
!
!    Input/output, integer ( kind = 4 ) A_INT(NROW,NCOL), the current 
!    integer or integer mod matrix.
!
...
!    Input, integer ( kind = 4 ) NROW, NCOL, the number of rows and columns.
!
!    Input, real ( kind = 8 ) SVAL, the multiplier to use if real arithmetic
!    is employed.
!
...
!
  if ( iform == 0 ) then
    ...
    if ( sval == 1.0D+00 ) then
      ...
      call r8_to_s_left ( sval, chrtmp3 )
    end if
...
!    rat/real/dec/int/mod arithmetic.
!
!    Input, integer ( kind = 4 ) ICOL1, the column to which the multiple
!    is to be added.
!
...
!
!    Output, real ( kind = 8 ) SVAL, the multiplier, if real arithmetic is used.
!
  implicit none
...
!
  if ( iform == 0 ) then
    ...
    call r8_read ( sval, line, prompt, ierror )
  else if ( iform == 2 ) then
...
!  Parameters:
!
!    Input/output, real ( kind = 8 ) A(NROW,NCOL), the real matrix.
!
!    Input/output, integer ( kind = 4 ) A_INT(NROW,NCOL), the current 
!    integer or integer mod matrix.
!
...
!
!    Output, integer ( kind = 4 ) IERROR, 0 an error occurred, 
!    1 no error occurred.
!
!    Input, integer ( kind = 4 ) BASE, the base for modular arithmetic.
!
...
!
      ...
      do kcol = jcol, ncol
        ...
        if ( iform == 0 ) then
          call rat_to_r8 ( iatop(irow,kcol), iabot(irow,kcol), atemp )
        else if ( iform == 1 ) then
          atemp = a(irow,kcol)
        else if ( iform == 2 ) then
          call dec_to_r8 ( iatop(irow,kcol), iabot(irow,kcol), atemp )
        else if ( iform == 3 ) then
          atemp = real ( a_int(irow,kcol), kind = 8 )
        else if ( iform == 4 ) then
          atemp = real ( a_int(irow,kcol), kind = 8 )
        end if
...
!
    ...
    call col_div ( a, a_int, iatop, iabot, ierror, iform, jcol, &
      nrow, ncol, sval, istop, isbot )
!
...
subroutine col_div ( a, a_int, iatop, iabot, ierror, iform, icol, &
  nrow, ncol, sval, istop, isbot )
...
!    Input, integer ( kind = 4 ) NROW, NCOL, the number of rows and columns.
!
!    Input, real ( kind = 8 ) SVAL, the real divisor.
!
!    Input, integer ( kind = 4 ) ISTOP, ISBOT, the fractional or 
!    decimal divisor.
!
  implicit none
...
!
  if ( iform == 0 ) then
    ...
    call r8_to_s_left ( sval, chrtmp2 )
    op = ' / '
...
!    Input, integer ( kind = 4 ) NROW, NCOL, the number of rows and columns.
!
!    Input, real ( kind = 8 ) SVAL, the real row multiplier.
!
!    Input, integer ( kind = 4 ) ISTOP, ISBOT, the decimal or fractional 
!    row multiplier.
!
  implicit none
...
!    Workspace, character ( len = 80 ) LINE, used to hold the user's input.
!
!    Output, real ( kind = 8 ) RVAL, the multiplier to use for real arithmetic.
!
  implicit none
...
!
  if ( iform == 0 ) then
    ...
    call r8_read ( rval, line, prompt, ierror )
  else if ( iform == 2 ) then
    call dec_read ( istop, isbot, line, prompt, ierror )
    if ( ierror == 0 ) then   
      call dec_round ( istop, isbot )
...
!
  ...
  if ( ierror /= 0 ) then
    ...
    write ( *, '(a)' ) '  Your ECO command could not be understood.'
    write ( *, '(a)' ) &
      '  The index of the second column "C2" did not make sense.'
    ierror = 1
    return
  end if
...
!
  do i = 1, nrow
    ...
    if ( iform == 0 ) then
      ...
      call r8_swap ( a(i,icol1), a(i,icol2) )
...
!
!    Input, integer ( kind = 4 ) IBOT1, IBOT2, the exponents of the numbers
!    to be added.
!
!    Output, integer ( kind = 4 ) ITOP, the coefficient of the result.
!
!    Input, integer ( kind = 4 ) ITOP1, ITOP2, the coefficients of the numbers
!    to be added.
!
  implicit none
...
!
  ...
  temp = log ( real ( abs ( itop1 ), kind = 8 ) ) &
       + log ( real ( abs ( itop2 ), kind = 8 ) )
...
subroutine dec_to_r8 ( itop, ibot, a )
...
!
!    Output, real ( kind = 8 ) A, the equivalent real value.
!
  implicit none
...
!    Input, integer ( kind = 4 ) BASE, the base for modular arithmetic.
!
!    Input/output, integer ( kind = 4 ) IFORM, 0/1/2/3/4, 
!    rat/real/dec/int/mod arithmetic.
!
  ...
  do i = 1, nrow
    do j = 1, ncol
      if ( jform == 0 ) then
        ...
        if ( iform == 1 ) then
          call r8_to_rat ( a(i,j), iatop(i,j), iabot(i,j) )
        else if ( iform == 2 ) then
        ...
        if ( iform == 0 ) then
          call rat_to_r8 ( iatop(i,j), iabot(i,j), a(i,j) )
        else if ( iform == 2 ) then
          call dec_to_r8 ( iatop(i,j), iabot(i,j), a(i,j) )
        else if ( iform == 3 ) then
          a(i,j) = real ( a_int(i,j), kind = 8 )
        else if ( iform == 4 ) then
          a(i,j) = real ( a_int(i,j), kind = 8 )
        end if
        ...
        if ( iform == 0 ) then
          ...
          call r8_to_dec ( a(i,j), iatop(i,j), iabot(i,j) ) 
        else if ( iform == 3 ) then
          iatop(i,j) = a_int(i,j)
          iabot(i,j) = 0
        else if ( iform == 4 ) then
        ...
        if ( iform == 0 ) then
          call rat_to_r8 ( iatop(i,j), iabot(i,j), r )
          a_int(i,j) = nint ( r )
        else if ( iform == 1 ) then
          a_int(i,j) = nint ( a(i,j) )
        else if ( iform == 2 ) then
          call dec_to_r8 ( iatop(i,j), iabot(i,j), r )
          a_int(i,j) = nint ( r )
        else if ( iform == 4 ) then
          a_int(i,j) = a_int(i,j)
        end if
...
!
  ...
  do i = 1, nrow
    do j = 1, ncol
      ...
      do while ( 10.0D+00 ** ( kmax - kmin ) <= abs ( a_int(i,j) ) )
        kmax = kmax + 1
      end do
...
!
  rlo = real ( ilo, kind = 8 ) - 0.5D+00
  rhi = real ( ihi, kind = 8 ) + 0.5D+00
!
...
!
  ...
  r = real ( seed, kind = 8 ) * 4.656612875D-10
!
...
!  Parameters:
!
!    Output, real ( kind = 8 ) A(MAXROW,MAXCOL), the current matrix.
!
!    Input/output, integer ( kind = 4 ) A_INT(NROW,NCOL), the current 
!    integer or integer mod matrix.
!
...
!
!    Output, integer ( kind = 4 ) IFORM, 0/1/2/3/4, 
!    rat/real/dec/int/mod arithmetic.
!
...
!    Input, integer ( kind = 4 ) NROW, NCOL, the number of rows and columns.
!
!    Output, real ( kind = 8 ) A(NROW,NCOL), the current matrix.
!
...
!
  if ( row1 == row2 .and. col1 == col2 ) then
    ...
    if ( iform == 0 ) then
      ...
      call r8_read ( a(row1,col1), line, prompt, ierror )
    else if ( iform == 2 ) then
...
!
    ...
    do i = row1, row2
      ...
      if ( iform == 0 ) then
        ...
        call r8_read ( a(i,col1), line, prompt, ierror )
    ...
    do i = row1, row2
      do j = col1, col2
        ...
        if ( iform == 0 ) then
          ...
          call r8_read ( a(i,j), line, prompt, ierror )
...
subroutine la_opt ( a, a_int, iabot, iatop, iform, nrow, ncol )
...
!  Parameters:
!
!    Input, real ( kind = 8 ) A(NROW,NCOL), the current matrix.
!
!    Input, integer ( kind = 4 ) A_INT(NROW,NCOL), the current 
!    integer or integer mod matrix.
!
  ...
  do i = 1, nrow
    do j = 1, ncol
      ...
      if ( iform == 0 ) then
        ...
        call r8_to_s_left ( a(i,j), chrtmp )
      else if ( iform == 2 ) then
        call dec_to_s_left ( iatop(i,j), iabot(i,j), chrtmp )
      else if ( iform == 3 ) then
    ...
    if ( izer == 0 ) then
      ...
      do j = 1, ncol
        ...
        if ( ( iform == 0 .and. iatop(i,j) /= 0 ) .or. &
          ...
          write ( *, '(a)' ) &
            '  Row ' // trim ( chrtmp1 ) // ' is entirely zero.'
          call i4_to_s_left ( i, chrtmp1 )
          write ( *, '(a)' ) &
            '  Row ' // trim ( chrtmp1 ) // ' occurs later, and has'
...
!
      do ii = 1, nrow
        ...
        if ( ii /= i ) then
          ...
          write ( *, '(a)' ) &
            '  This means that all other entries of that ' // &
            'column should be zero.'
 
          if ( iform == 0 ) then
            call rat_to_s_left ( iatop(ii,j), iabot(ii,j), chrtmp )
          else if ( iform == 1 ) then
            call r8_to_s_left ( a(ii,j), chrtmp )
          else if ( iform == 2 ) then
...
!  Parameters:
!
!    Input/output, integer ( kind = 4 ) NROW, NCOL, the number of rows and 
!    columns of the matrix.  On output, NCOL has been increased to NCOL+NROW.
!
!    Input/output, real ( kind = 8 ) A(NROW,NCOL), the current matrix.
!
!    Input/output, integer ( kind = 4 ) A_INT(NROW,NCOL), the current 
...
!
  ...
  if ( iform == 0 ) then
    ...
    call r8_print ( a, nrow, ncol, title )
  else if ( iform == 2 ) then
...
subroutine r8_print ( a, nrow, ncol, title )
...
subroutine r8_read ( rval, line, prompt, ierror )
...
!  Parameters:
!
!    Output, real ( kind = 8 ) RVAL, the real value found in LINE.
!
!    Workspace, character ( len = 80 ) LINE, used to hold the user's input.
!
...
!
  call s_to_r8 ( line, top, ierror, lchar )
!
...
!
  if ( lchar+1 < len ( line ) ) then
    if ( line(lchar+1:lchar+1) == '/' ) then
      lchar = lchar + 1
      call s_chop ( line, 1, lchar )
      call s_to_r8 ( line, bot, ierror, lchar )
      if ( bot == 0.0D+00 ) then
        bot = 1.0D+00
      end if
    end if
...
!    is desired.
!
!    Output, integer ( kind = 4 ) ITOP, IBOT, the approximate decimal 
...
subroutine r8_to_rat ( a, iatop, iabot )
...
!  Parameters:
!
!    Input, real ( kind = 8 ) A, the real value to be converted.
!
...
!    real ( kind = 8 ) arithmetic and then converted back to a rational value.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
  rtop1 = real ( jtop1, kind = 8 ) * real ( jbot2, kind = 8 )
  ...
  rtop2 = real ( jtop2, kind = 8 ) * real ( jbot1, kind = 8 )
  ...
  rtop3 = real ( jtop1, kind = 8 ) + real ( jtop2, kind = 8 )
...
!
  rbot = real ( jbot1, kind = 8 ) * real ( jbot2, kind = 8 ) &
    * real ( jbot3, kind = 8 )
...
!
  if ( ierror /= 0 ) then
    ierror = 0
    rval1 = real ( itop1, kind = 8 ) / real ( ibot1, kind = 8 )
    rval2 = real ( itop2, kind = 8 ) / real ( ibot2, kind = 8 )
    rval = rval1 + rval2
    call r8_to_rat ( rval, itop, ibot )
  end if
!
...
!
  rtop = real ( jtop1, kind = 8 ) * real ( jtop2, kind = 8 )
...
subroutine rat_to_r8 ( iatop, iabot, a )
...
!
!    Output, real ( kind = 8 ) A, the value of the rational quantity.
!
  implicit none
  ...
  if ( iabot == 0 ) then
    ...
    a = real ( iatop, kind = 8 ) / real ( iabot, kind = 8 )
  end if
...
!
  if ( iform == 0 ) then
    ...
    do j = 1, ncol
      ...
      call rat_add ( ibot, iabot(irow1,j), isbot2, itop, iatop(irow1,j), &
        istop2 )
...
!    rat/real/dec/int/mod arithmetic.
!
!    Input, integer ( kind = 4 ) IROW1, the row to which the multiple
!    is to be added.
!
...
!
      ...
      do krow = irow, nrow
        ...
        if ( iform == 0 ) then
          call rat_to_r8 ( iatop(krow,jcol), iabot(krow,jcol), atemp )
        else if ( iform == 1 ) then
          atemp = a(krow,jcol)
        else if ( iform == 2 ) then
          call dec_to_r8 ( iatop(krow,jcol), iabot(krow,jcol), atemp )
        else if ( iform == 3 ) then
          atemp = real ( a_int(krow,jcol), kind = 8 )
        else if ( iform == 4 ) then
          atemp = real ( a_int(krow,jcol), kind = 8 )
        end if
...
!    Workspace, character ( len = 80 ) LINE, used to hold the user's input.
!
!    Output, real ( kind = 8 ) SVAL, the real divisor.
!
  implicit none
...
!
  do j = 1, ncol
    ...
    if ( iform == 0 ) then
      ...
      call r8_swap ( a(irow1,j), a(irow2,j) )
...
!
  ...
  if ( iform == 0 ) then
    ...
    if ( ierror /= 0 ) then
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 'VALUE_READ - Error!'
      write ( *, '(a)' ) '  R8_read returned error flag!'
      return
    end if
