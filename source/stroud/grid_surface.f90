...
!*****************************************************************************80
!
!! GRID_SURFACE uses DISLIN to display Z(X,Y) data as a surface.
!
!  Licensing:
!
...
! use dislin
  ...
  write ( *, '(a)' ) '  Use DISLIN to plot Z(X,Y) data as a surface.'
!
!  Read the data.
!
  call r4mat_header_read ( 'grid_data.txt', m, nn )

  allocate ( xyz(1:m,1:nn) )

  call r4mat_data_read ( 'grid_data.txt', m, nn, xyz )
!
!  Split the data.
!
  n = 41
...
!  Define the title.
!
  call titlin ( 'Surface plot of 41x41 table Z(X,Y)', 1 )
  call title ( )
!
