...
!  Parameters:
!
!    Input, integer ( kind = 4 ) ILO, IHI, the minimum and maximum acceptable
!    values.
!
...
!
!    Input/output, integer ( kind = 4 ) P(N), a permutation, in standard 
!    index form.
!
...
!    Input, integer ( kind = 4 ) N, the number of words.
!
!    Input/output, integer ( kind = 4 ) PERM(N), a permutation, the ordering 
...
!    Input, character ( len = * ) WORD(N), a list of words.
!
!    Input/output, integer ( kind = 4 ) PERM_BEST(N), a permutation, the
